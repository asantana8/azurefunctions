...
async function gerarCodigoBarras() {
    ...
    if (gerarButton) gerarButton.disabled = true;
    ...
    try {
        ...
        // Chamar a API
        const response = await fetch('http://localhost:7168/api/barcode-generate', {
        method: 'POST',
        headers: {
...
async function validarCodigo() {
    ...
    const response = await fetch('http://localhost:7017/api/barcode-validate', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
